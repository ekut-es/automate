...
uint64_t get_time_ns() {
  ...
  clock_gettime(CLOCK_MONOTONIC, &measured_time);
   
  sec = measured_time.tv_sec;
  nsec = measured_time.tv_nsec;
  
  nsec = nsec + 1000000000 * sec;
  ...
  do
    {
      ...
      User_Time = ((double)(end_time - start_time)) / (1e9);
